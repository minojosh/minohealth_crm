# -*- coding: utf-8 -*-
"""unified_speech_service.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1iQz_c8thrazZTRxZGIwQXQByA3cIeXTK
"""
...
# !wget https://huggingface.co/novateur/WavTokenizer-large-speech-75token/resolve/main/wavtokenizer_large_speech_320.ckpt
!wget https://huggingface.co/novateur/WavTokenizer-large-speech-75token/resolve/main/wavtokenizer_large_speech_320_v2.ckpt
# !wget https://huggingface.co/novateur/WavTokenizer-medium-speech-75token/resolve/main/wavtokenizer_medium_speech_320_24k.ckpt
...
class UnifiedSpeechConfig:
    ...
    def __init__(self):
        ...
        self.WAV_TOKENIZER_MODEL = "wavtokenizer_large_speech_320_v2.ckpt"
        self.MIN_AUDIO_SAMPLES = 16000 * 3  # 10 seconds at 16kHz
...
class AudioProcessor:
    ...
    def add_audio(self, audio_array, client_id):
        ...
        self.logger.info(f"Client {client_id}: Buffer size after adding audio: {len(self.buffer)} samples")
    ...
    def get_audio(self):
        ...
        self.buffer = np.array([], dtype=np.float32)
        self.logger.info(f"Retrieved {len(audio)} samples for processing, buffer cleared")
        return audio

    def get_temp_audio(self):
        audio = self.temp_buffer.copy()
        self.temp_buffer = np.array([], dtype=np.float32)
        self.logger.info(f"Retrieved {len(audio)} samples from temp buffer")
        return audio
    ...
    def update_context(self, transcription):
        if transcription:
            ...
            self.logger.info(f"Context updated, now contains {len(words)} words")
    ...
    def register_client(self, client_id):
        if client_id not in self.clients:
            ...
            self.logger.info(f"Client {client_id}: Request #{self.clients[client_id]['request_count']}")

    def update_client_stats(self, client_id, samples_count):
        if client_id in self.clients:
            self.clients[client_id]["total_audio_samples"] += samples_count
            self.logger.info(f"Client {client_id}: Total samples received: {self.clients[client_id]['total_audio_samples']}")
        ...
        if 'command' in data and data['command'] == 'finish':
            ...
            if len(final_audio) > 0:
                ...
                if transcription:
                    ...
                    app.state.logger.info(f"Client {client_id}: {transcription}")
