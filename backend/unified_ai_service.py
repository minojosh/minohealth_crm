...
- Moremi AI conversational services
...
class ConfigManager:
    ...
    def get_moremi_url(self) -> Optional[str]:
        """Get Moremi API URL from environment variables."""
        return os.getenv("MOREMI_API_BASE_URL")


class AIService:
    """Base class for AI services providing unified interface to Moremi, TTS and STT."""
    ...
    def __init__(self, config_manager: Optional[ConfigManager] = None):
        ...
        self.moremi_client = self._init_moremi_client()
    ...
    def _init_moremi_client(self) -> ConversationManager:
        """Initialize Moremi client."""
        base_url = self.config_manager.get_moremi_url()
        logger.info(f"Initializing Moremi client with URL: {base_url}")
        return ConversationManager(base_url=base_url)
    ...
    def process_with_prompt(self, prompt_key: str, user_input: str, 
        ...
        Returns:
            The response from Moremi
        """
        # Get the appropriate system prompt
        system_prompt = self.config_manager.get_prompt(prompt_key)
        logger.info(f"Using prompt key '{prompt_key}' for processing")
        
        # Configure the Moremi client
        self.moremi_client.custom_params["system_prompt"] = system_prompt
        
        # Process the input and get response
        self.moremi_client.add_user_message(text=user_input)
        response = self.moremi_client.get_assistant_response(
...
class MedicalAssistantService(AIService):
    ...
    def extract_data(self, transcript: str) -> Dict[str, Any]:
        ...
        # Send the request to Moremi for processing
        logger.info(f"Extracting data from transcript: {transcript[:100]}...")
        
        # Configure extraction-specific parameters
        self.moremi_client.custom_params["system_prompt"] = "You are an expert medical data extraction system."
        
        # Add the user message with extraction prompt
        self.moremi_client.add_user_message(text=extraction_prompt)
    ...
    def generate_soap_note(self, conversation_text: str) -> Dict[str, Any]:
        ...
        self.moremi_client.custom_params["system_prompt"] = "You are an expert medical documentarian."
        
        # Process the input and get response
        self.moremi_client.add_user_message(text=prompt)
        soap_response = self.moremi_client.get_assistant_response(
            max_tokens=800,
            temperature=0.3,
            stream=False
        )
